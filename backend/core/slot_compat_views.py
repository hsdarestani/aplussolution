...
def _assignment_pairs(start=None, end=None):
    """Return unique (worker, shift) assignments from native slots plus legacy Shift.worker."""
    slot_qs = ShiftSlot.objects.filter(
        status=ShiftSlot.Status.CLAIMED,
        worker__isnull=False,
    ).exclude(shift__status=Shift.Status.CANCELLED).select_related(
        'worker__user', 'shift__client', 'shift__location', 'shift__position', 'shift__order'
    )
    legacy_qs = Shift.objects.filter(worker__isnull=False).exclude(status=Shift.Status.CANCELLED).select_related(
...
def _schedule_findings(date_from=None, date_to=None):
    now = timezone.now()
    start = date_from or now - timezone.timedelta(days=7)
    end = date_to or now + timezone.timedelta(days=90)
    ...
    for worker_rows in by_worker.values():
        worker_rows.sort(key=lambda item: item[1].starts_at)
        for (worker, previous), (_, current) in zip(worker_rows, worker_rows[1:]):
    ...
    for availability in unavailable_rows:
        matches = [
            shift for _, shift in by_worker.get(availability.worker_id, [])
    ...
    for order in orders:
        ...
        open_count = ShiftSlot.objects.filter(
            shift__order=order,
            status=ShiftSlot.Status.OPEN,
            worker__isnull=True,
        ).exclude(shift__status=Shift.Status.CANCELLED).count()
    ...
    month_pairs = _assignment_pairs(_aware_start(month_start), _aware_start(month_end))
    minutes_by_worker = defaultdict(int)
    for worker, shift in month_pairs:
...
def operations_overview(request):
    ...
    if _is_manager(user):
        ...
        month_pairs = _assignment_pairs(_aware_start(current_month_start), _aware_start(current_month_end))
        ...
        data.update({
            **findings,
            ...
            'missing_clock_outs': TimeEntry.objects.filter(clock_out__isnull=True, clock_in__lt=now - timezone.timedelta(hours=16)).count(),
            'contracts_due_30': Contract.objects.filter(
                ends_on__range=(timezone.localdate(), timezone.localdate() + timezone.timedelta(days=30)),
...
def schedule_quality(request):
    ...
    if denied:
        return denied
    try:
        from .advanced_views import _as_dt
...
def swap_create(request):
    ...
    owns_slot = ShiftSlot.objects.filter(
        shift=shift,
        worker=worker,
        status=ShiftSlot.Status.CLAIMED,
    ).exists()
    if not owns_slot and shift.worker_id != worker.id:
        ...
        with transaction.atomic():
            if decision == ShiftSwapRequest.Status.APPROVED:
                ...
                if ShiftSlot.objects.filter(
                    ...
                    status=ShiftSlot.Status.CLAIMED,
                ).exists() or obj.shift.worker_id == obj.offered_to_id:
                ...
                if slot is None and obj.shift.worker_id == obj.requested_by_id:
                    slot = ShiftSlot.objects.select_for_update().filter(
                        shift=obj.shift,
                        status=ShiftSlot.Status.OPEN,
                        worker__isnull=True,
                ...
                if slot is not None:
                    ...
                    slot.save(update_fields=[
                        'worker', 'status', 'source', 'claimed_at', 'released_at', 'updated_at'
                    ])
