...
def _allowed_target(target: str) -> str:
    """Only allow OAuth callbacks back into the configured A+ Solution app."""
    app = urlparse(settings.APP_URL)
    candidate = urlparse(target or '')
    if not candidate.scheme or not candidate.netloc:
        return settings.APP_URL.rstrip('/') + '/auth/callback'
    if candidate.scheme != app.scheme or candidate.netloc != app.netloc:
        return settings.APP_URL.rstrip('/') + '/auth/callback'
    return target


def _state(provider, target):
    return signing.dumps(
        {'provider': provider, 'target': _allowed_target(target), 'nonce': get_random_string(24)},
        salt='social-oauth',
    )
...
def start(provider, target):
    ...
    if provider == 'google':
        if not settings.GOOGLE_OAUTH_CLIENT_ID or not settings.GOOGLE_OAUTH_CLIENT_SECRET:
            raise ValueError('Google-Anmeldung ist auf dem Server noch nicht vollständig konfiguriert.')
        if not settings.GOOGLE_OAUTH_REDIRECT_URI:
            raise ValueError('Google OAuth Redirect-URI fehlt auf dem Server.')
        params = {
    ...
    if provider == 'apple':
        if not settings.APPLE_SERVICE_ID or not settings.APPLE_TEAM_ID:
            raise ValueError('Apple-Anmeldung ist auf dem Server noch nicht vollständig konfiguriert.')
        if not settings.APPLE_OAUTH_REDIRECT_URI:
            raise ValueError('Apple OAuth Redirect-URI fehlt auf dem Server.')
        params = {
...
def apple_client_secret():
    ...
    if not key:
        raise ValueError('Apple Private Key fehlt.')
    if not settings.APPLE_KEY_ID:
        raise ValueError('Apple Key ID fehlt.')
    now = int(time.time())
    return jwt.encode(
        {
...
def finish(provider, code, state):
    ...
    if data['provider'] != provider:
        raise ValueError('Ungültiger OAuth-Status')

    profile = exchange(provider, code)
    email = profile.get('email')
    if not email:
        raise ValueError('Der Anbieter hat keine E-Mail-Adresse geliefert.')
    ...
    target = _allowed_target(data.get('target', ''))
