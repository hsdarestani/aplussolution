...
function appShellTransforms(): Plugin {
  ...
  return {
    ...
    transform(code, id) {
      ...
      if (!code.includes(viewStateMarker)) this.error('App view-state marker changed; update app-shell-transforms before building.');
      return { code: `import { useViewRouting } from './viewRouting';\n${code.replace(viewStateMarker,'const [view, setView] = useViewRouting(user?.role);')}`, map: null };
    },
    ...
    load(id) {
      ...
      return `
        ...
        export default function LazyAppFeature(props) {
          const fallback = React.createElement('div',{ className: 'loader', role: 'status', 'aria-live': 'polite' },React.createElement('p', null, 'Daten werden geladen …'));
          return React.createElement(Suspense,{ fallback },React.createElement(Feature, props));
        }
      `;
    },
  };
...
export default defineConfig({
  plugins: [
    ...
    VitePWA({
      ...
      manifest: {
        name: 'A+ Solution Workforce', short_name: 'A+ Solution', description: 'Personal, Einsätze, Zeiten und Verträge.',
        theme_color: '#0b1f4d', background_color: '#f4f7fb', display: 'standalone', lang: 'de',
        icons: [
