...
export default function PortalAccessPanel(){
  ...
  const bulkEligible=Number(counts.not_activated||0)+Number(counts.invited||0);
  ...
  async function bulk(){setBusy('bulk');try{const r:any=await api('workers/bulk-invite/',{method:'POST',body:'{}'});const links=(r.results||[]).filter((x:any)=>x.activation_url&&!String(x.email||'').toLowerCase().endsWith('@sync.invalid')).map((x:any)=>`${x.email}: ${x.activation_url}`).join('\n');if(links){await navigator.clipboard?.writeText(links);setToast(`${r.count} Einladung(en) erstellt. Links wurden kopiert.`);}else setToast(`${r.count} Einladung(en) verarbeitet.`);await load();}catch(e:any){setToast(e.message);}finally{setBusy('');}}
  return <section className="portal-access-panel">
    ...
    <IonAlert
      ...
      message={`Für bis zu ${bulkEligible} Mitarbeiter werden Aktivierungen verarbeitet. Bereits aktive Zugänge und Migrationsdatensätze bleiben unberührt.`}
      buttons={[
