...
const noticeLabels: Record<string, string> = {
  early_clock_in: 'Zu früh eingestempelt', late_clock_in: 'Zu spät eingestempelt',
  early_clock_out: 'Zu früh ausgestempelt', late_clock_out: 'Zu spät ausgestempelt',
  wrong_location: 'Falscher Standort', missed_clock_in: 'Einstempeln fehlt', missed_clock_out: 'Ausstempeln fehlt',
  no_show: 'Nicht erschienen', not_scheduled: 'Nicht eingeplant', photo_missing: 'Foto fehlt',
  break_missed: 'Pause fehlt', break_short: 'Pause zu kurz', attestation_missing: 'Bestätigung fehlt', terminal_denied: 'Terminal abgelehnt',
};
...
export default function AttendanceV4({ user }: { user: User }) {
  ...
  async function load() {
    try {
      ...
      if (manager && !policy) setPolicy(main.policies?.[0] || null);
    } catch (error: any) {
  ...
  async function clock(kind: 'in' | 'out') {
    ...
    try {
      ...
      if (kind === 'out' && (result.attestation_required?.break || result.attestation_required?.end_of_shift)) {
        setAttest({ entry: result.id, break: result.attestation_required.break, end: result.attestation_required.end_of_shift, breaks_taken: true, shift_ok: true, note: '' });
      }
      await load();
    } catch (error: any) {
      setToast(error.message);
    ...
    try {
      if (attest.break) {
        await api(`attendance/entries/${attest.entry}/attestation/`, {
          method: 'POST', body: JSON.stringify({ kind: 'break', answers: { breaks_taken: !!attest.breaks_taken }, note: attest.breaks_taken ? '' : attest.note }),
        });
      }
      if (attest.end) {
        await api(`attendance/entries/${attest.entry}/attestation/`, {
          method: 'POST', body: JSON.stringify({ kind: 'end_of_shift', answers: { shift_completed_as_planned: !!attest.shift_ok }, note: attest.note }),
        });
      }
      setAttest(undefined);
  ...
  async function submitCorrection() {
    ...
    const result = await mutate(`attendance/entries/${correction.entry.id}/correction/`, {
      clock_in: correction.clock_in, clock_out: correction.clock_out, reason: correction.reason,
  ...
  async function closeRunning(id: string) {
    const reason = window.prompt('Grund für das Beenden des Timers:');
    if (!reason) return;
    await mutate(`attendance/entries/${id}/close/`, { reason }, 'Timer beendet und zur Prüfung markiert.');
  }
  ...
  async function savePolicy() {
    ...
    delete payload.id; delete payload.created_at; delete payload.updated_at; delete payload.location_name;
    const result = await mutate(policy.id ? `attendance-policies/${policy.id}/` : 'attendance-policies/', payload, 'Attendance Policy gespeichert.', policy.id ? 'PATCH' : 'POST');
    if (result) setPolicy(result);
  }

  async function createTerminal() {
    if (!terminal?.name || !terminal?.location) { setToast('Name und Einsatzort auswählen.'); return; }
  ...
  if (manager) {
    ...
    return <div className="attendance-v4">
      <section className="att-v4-hero manager">
        <div><small>ATTENDANCE CONTROL CENTER</small><h1>Arbeitszeit, Abweichungen & Terminals</h1><p>Attendance Notices priorisieren, Pausenregeln steuern und Time Clock Terminals verwalten.</p></div>
      ...
      <section className="att-v4-panel notice-center">
        <div className="att-v4-section-head"><div><small>ATTENDANCE NOTICES</small><h2>Abweichungen zuerst bearbeiten</h2></div><IonBadge color={notices.some((n:any)=>n.severity==='critical')?'danger':'medium'}>{notices.length} offen</IonBadge></div>
        {notices.length ? notices.map((item: any) => <article className={`att-notice ${item.severity}`} key={item.id}>
          ...
          <div className="att-notice-copy"><div><IonBadge color={item.severity==='critical'?'danger':item.severity==='warning'?'warning':'medium'}>{item.severity}</IonBadge><span>{item.worker_name}</span></div><h3>{noticeLabels[item.notice_type] || item.notice_type}</h3><p>{item.shift_title || 'Arbeitszeit'}{item.location_name ? ` · ${item.location_name}` : ''}{item.value_minutes != null ? ` · ${item.value_minutes} Min.` : ''}</p><small>{dateTime(item.detected_at)}</small></div>
          <div className="att-notice-actions">{item.status === 'open' && <IonButton size="small" fill="outline" onClick={() => void noticeAction(item.id,'acknowledge')}>Gesehen</IonButton>}<IonButton size="small" onClick={() => void noticeAction(item.id,'resolve')}>Erledigt</IonButton><IonButton size="small" fill="clear" color="medium" onClick={() => void noticeAction(item.id,'dismiss')}>Verwerfen</IonButton></div>
      ...
      <div className="att-v4-admin-grid">
        <section className="att-v4-panel">
          ...
          <IonSelect fill="outline" label="Policy" labelPlacement="floating" value={policy?.id || ''} onIonChange={(e) => { const found=data.policies?.find((p:any)=>p.id===val(e)); setPolicy(found || {name:'Neue Policy',active:true,priority:0}); }}><IonSelectOption value="">Neue Policy</IonSelectOption>{data.policies?.map((p:any)=><IonSelectOption key={p.id} value={p.id}>{p.name}{p.location_name?` · ${p.location_name}`:' · Global'}</IonSelectOption>)}</IonSelect>
          {policy && <div className="att-policy-form">
            ...
            <IonSelect fill="outline" label="Einsatzort" labelPlacement="floating" value={policy.location || ''} onIonChange={(e)=>setPolicy({...policy,location:val(e)||null})}><IonSelectOption value="">Global</IonSelectOption>{locations.map((l:any)=><IonSelectOption key={l.id} value={l.id}>{l.name}</IonSelectOption>)}</IonSelect>
            <IonInput fill="outline" type="number" label="Früh einchecken (Min.)" labelPlacement="floating" value={policy.early_clock_in_minutes} onIonInput={(e)=>setPolicy({...policy,early_clock_in_minutes:Number(val(e)||0)})}/>
            <ModeSelect label="Zu frühes Einchecken" value={policy.early_clock_in_mode} onChange={(v)=>setPolicy({...policy,early_clock_in_mode:v})}/>
            <IonInput fill="outline" type="number" label="Late Grace (Min.)" labelPlacement="floating" value={policy.late_clock_in_grace_minutes} onIonInput={(e)=>setPolicy({...policy,late_clock_in_grace_minutes:Number(val(e)||0)})}/>
            <IonInput fill="outline" type="number" label="No-show nach (Min.)" labelPlacement="floating" value={policy.no_show_after_minutes} onIonInput={(e)=>setPolicy({...policy,no_show_after_minutes:Number(val(e)||0)})}/>
            <ModeSelect label="Standort beim Einchecken" value={policy.clock_in_location_mode} onChange={(v)=>setPolicy({...policy,clock_in_location_mode:v})}/>
            <ModeSelect label="Standort beim Auschecken" value={policy.clock_out_location_mode} onChange={(v)=>setPolicy({...policy,clock_out_location_mode:v})}/>
            <IonInput fill="outline" type="number" label="Pflichtpause ab (Min.)" labelPlacement="floating" value={policy.required_break_after_minutes} onIonInput={(e)=>setPolicy({...policy,required_break_after_minutes:Number(val(e)||0)})}/>
            <IonInput fill="outline" type="number" label="Pflichtpause (Min.)" labelPlacement="floating" value={policy.required_break_minutes} onIonInput={(e)=>setPolicy({...policy,required_break_minutes:Number(val(e)||0)})}/>
            <Toggle label="Pause bezahlt" checked={!!policy.default_break_paid} onChange={(v)=>setPolicy({...policy,default_break_paid:v})}/>
            <Toggle label="Fehlende unbezahlte Pause automatisch abziehen" checked={!!policy.auto_deduct_unpaid_breaks} onChange={(v)=>setPolicy({...policy,auto_deduct_unpaid_breaks:v})}/>
            <Toggle label="Pausenbestätigung beim Ausstempeln" checked={!!policy.break_attestation_required} onChange={(v)=>setPolicy({...policy,break_attestation_required:v})}/>
            <Toggle label="Schichtende bestätigen" checked={!!policy.end_of_shift_attestation_required} onChange={(v)=>setPolicy({...policy,end_of_shift_attestation_required:v})}/>
            <Toggle label="Terminal-Foto beim Einstempeln" checked={!!policy.terminal_photo_clock_in} onChange={(v)=>setPolicy({...policy,terminal_photo_clock_in:v})}/>
            <Toggle label="Terminal-Foto beim Ausstempeln" checked={!!policy.terminal_photo_clock_out} onChange={(v)=>setPolicy({...policy,terminal_photo_clock_out:v})}/>
        ...
        <section className="att-v4-panel">
          ...
          <div className="terminal-list">{data.terminals?.map((item:any)=><article key={item.id}><div><b>{item.name}</b><span>{item.location_name}</span><small>{item.active?'Aktiv':'Deaktiviert'} · zuletzt {item.last_seen_at?dateTime(item.last_seen_at):'noch nie'}</small></div><div><IonBadge color={item.photo_clock_in||item.photo_clock_out?'primary':'medium'}><IonIcon icon={cameraOutline}/> Foto {item.photo_clock_in?'IN':''}{item.photo_clock_out?' OUT':''}</IonBadge><IonButton size="small" fill="outline" onClick={()=>void rotateTerminal(item)}>Secret erneuern</IonButton></div></article>)}</div>
      ...
      <section className="att-v4-panel">
        ...
        {data.long_running_entries?.map((item:any)=><Row key={item.id} title={item.worker_name} subtitle="Laufender Timer" meta={`${minutesLabel(runningMinutes(item.clock_in,now))} Std. aktiv`} actions={<IonButton size="small" color="danger" onClick={()=>void closeRunning(item.id)}>Timer beenden</IonButton>}/>) }
      </section>

      <AbsencePanel rows={timeOff} manager onDecision={decideAbsence}/>

      <IonModal isOpen={!!terminal} onDidDismiss={()=>setTerminal(undefined)}><div className="att-v4-modal"><small>TIME CLOCK TERMINAL</small><h2>Terminal einrichten</h2><IonInput fill="outline" label="Name" labelPlacement="floating" value={terminal?.name} onIonInput={(e)=>setTerminal({...terminal,name:val(e)})}/><IonSelect fill="outline" label="Einsatzort" labelPlacement="floating" value={terminal?.location} onIonChange={(e)=>setTerminal({...terminal,location:val(e)})}>{locations.map((l:any)=><IonSelectOption key={l.id} value={l.id}>{l.name}</IonSelectOption>)}</IonSelect><Toggle label="Foto beim Einstempeln" checked={!!terminal?.photo_clock_in} onChange={(v)=>setTerminal({...terminal,photo_clock_in:v})}/><Toggle label="Foto beim Ausstempeln" checked={!!terminal?.photo_clock_out} onChange={(v)=>setTerminal({...terminal,photo_clock_out:v})}/><div className="att-v4-modal-actions"><IonButton fill="outline" onClick={()=>setTerminal(undefined)}>Abbrechen</IonButton><IonButton disabled={busy} onClick={()=>void createTerminal()}>Anlegen</IonButton></div></div></IonModal>
      <IonModal isOpen={!!terminalSecret} onDidDismiss={()=>setTerminalSecret(undefined)}><div className="att-v4-modal secret-modal"><small>SECRET · NUR EINMAL SICHTBAR</small><h2>{terminalSecret?.name || 'Terminal'}</h2><p>Terminal URL:</p><code>{`${window.location.origin}/terminal/${terminalSecret?.public_id}`}</code><p>Terminal Secret:</p><code>{terminalSecret?.terminal_token}</code><p>Beides auf dem Kiosk-Gerät hinterlegen.</p><IonButton onClick={()=>setTerminalSecret(undefined)}>Verstanden</IonButton></div></IonModal>
  ...
  return <div className="attendance-v4 worker">
    ...
    <IonModal isOpen={!!attest} onDidDismiss={()=>setAttest(undefined)}><div className="att-v4-modal"><small>BESTÄTIGUNG</small><h2>Schicht abschließen</h2>{attest?.break && <label className="att-question"><IonCheckbox checked={!!attest.breaks_taken} onIonChange={(e)=>setAttest({...attest,breaks_taken:e.detail.checked})}/><span>Ich habe meine vorgesehenen Pausen genommen.</span></label>}{attest?.end && <label className="att-question"><IonCheckbox checked={!!attest.shift_ok} onIonChange={(e)=>setAttest({...attest,shift_ok:e.detail.checked})}/><span>Die Schicht wurde wie geplant beendet.</span></label>}<IonTextarea fill="outline" label="Hinweis / Grund" labelPlacement="floating" value={attest?.note} onIonInput={(e)=>setAttest({...attest,note:val(e)})}/><div className="att-v4-modal-actions"><IonButton fill="outline" onClick={()=>setAttest(undefined)}>Später</IonButton><IonButton disabled={busy} onClick={()=>void submitAttestation()}>Bestätigen</IonButton></div></div></IonModal>

    <IonModal isOpen={!!correction} onDidDismiss={()=>setCorrection(undefined)}><div className="att-v4-modal"><small>ARBEITSZEIT KORRIGIEREN</small><h2>Korrektur anfragen</h2><IonInput fill="outline" type="datetime-local" label="Beginn" labelPlacement="floating" value={correction?.clock_in} onIonInput={(e)=>setCorrection({...correction,clock_in:val(e)})}/><IonInput fill="outline" type="datetime-local" label="Ende" labelPlacement="floating" value={correction?.clock_out} onIonInput={(e)=>setCorrection({...correction,clock_out:val(e)})}/><IonTextarea fill="outline" label="Grund" labelPlacement="floating" value={correction?.reason} onIonInput={(e)=>setCorrection({...correction,reason:val(e)})}/><div className="att-v4-modal-actions"><IonButton fill="outline" onClick={()=>setCorrection(undefined)}>Abbrechen</IonButton><IonButton disabled={busy} onClick={()=>void submitCorrection()}>Anfrage senden</IonButton></div></div></IonModal>
    <IonModal isOpen={absence !== undefined} onDidDismiss={()=>setAbsence(undefined)}><div className="att-v4-modal"><small>ABWESENHEIT</small><h2>Antrag senden</h2><IonInput fill="outline" type="date" label="Von" labelPlacement="floating" value={absence?.starts_on} onIonInput={(e)=>setAbsence({...absence,starts_on:val(e)})}/><IonInput fill="outline" type="date" label="Bis" labelPlacement="floating" value={absence?.ends_on} onIonInput={(e)=>setAbsence({...absence,ends_on:val(e)})}/><IonTextarea fill="outline" label="Grund / Hinweis" labelPlacement="floating" value={absence?.reason} onIonInput={(e)=>setAbsence({...absence,reason:val(e)})}/><div className="att-v4-modal-actions"><IonButton fill="outline" onClick={()=>setAbsence(undefined)}>Abbrechen</IonButton><IonButton disabled={busy} onClick={()=>void requestAbsence()}>Antrag senden</IonButton></div></div></IonModal>
    <IonToast isOpen={!!toast} message={toast} duration={4500} onDidDismiss={()=>setToast('')}/>
  </div>;
}

function ModeSelect({label,value,onChange}:{label:string;value:string;onChange:(v:string)=>void}) { return <IonSelect fill="outline" label={label} labelPlacement="floating" value={value} onIonChange={(e)=>onChange(String(val(e)))}><IonSelectOption value="off">Aus</IonSelectOption><IonSelectOption value="warn">Hinweis</IonSelectOption><IonSelectOption value="block">Blockieren</IonSelectOption></IonSelect>; }
function Toggle({label,checked,onChange}:{label:string;checked:boolean;onChange:(v:boolean)=>void}) { return <label className="att-toggle"><span>{label}</span><IonToggle checked={checked} onIonChange={(e)=>onChange(e.detail.checked)}/></label>; }
function Stat({label,value,suffix,danger}:{label:string;value:any;suffix?:string;danger?:boolean}) { return <div className={`att-v4-stat ${danger?'danger':''}`}><small>{label}</small><strong>{value}</strong>{suffix&&<span>{suffix}</span>}</div>; }
function Empty({text}:{text:string}) { return <div className="att-v4-empty">{text}</div>; }
function Row({title,subtitle,meta,actions}:{title:string;subtitle?:string;meta?:string;actions?:React.ReactNode}) { return <div className="att-v4-row"><div><b>{title}</b><span>{subtitle}</span><small>{meta}</small></div><div>{actions}</div></div>; }
function AbsencePanel({rows,manager,onDecision,onNew}:{rows:any[];manager?:boolean;onDecision?:(id:string,status:'approved'|'rejected')=>void;onNew?:()=>void}) { return <section className="att-v4-panel"><div className="att-v4-section-head"><div><small>ABWESENHEIT</small><h2>{manager?'Anträge':'Meine Abwesenheiten'}</h2></div>{onNew&&<IonButton size="small" fill="outline" onClick={onNew}>Neue Abwesenheit</IonButton>}</div>{rows.length?rows.map((item:any)=><Row key={item.id} title={item.worker_name||'Mein Antrag'} subtitle={`${dateOnly(item.starts_on)} – ${dateOnly(item.ends_on)} · ${item.reason||'Ohne Hinweis'}`} meta={item.status} actions={manager&&item.status==='pending'&&onDecision?<><IonButton size="small" onClick={()=>onDecision(item.id,'approved')}>Genehmigen</IonButton><IonButton size="small" fill="outline" color="danger" onClick={()=>onDecision(item.id,'rejected')}>Ablehnen</IonButton></>:<IonBadge color={item.status==='approved'?'success':item.status==='rejected'?'danger':'warning'}>{item.status}</IonBadge>}/>):<Empty text="Keine Abwesenheitsanträge."/>}</section>; }
