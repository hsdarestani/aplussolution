...
export default function ScheduleDatePicker({ value, onSelect, onClose }: {
  value: string; onSelect: (date: string) => void; onClose: () => void;
}) {
  ...
  return <div className="wiw-sheet-backdrop wiw-calendar-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-label="Schichtdatum" className="wiw-calendar-sheet" onKeyDown={event => { if (event.key === 'Escape') onClose(); }}>
      <header><b>Schichtdatum</b><button type="button" onClick={onClose}>Abbrechen</button></header>
