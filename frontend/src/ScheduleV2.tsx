...
export default function ScheduleV2({user}:{user:User}) {
  ...
  const avatarUrls=useMemo(()=>user.role==='client'?[]:Array.from(new Set(rows.flatMap((row:any)=>(row.assigned_workers||[]).map((worker:any)=>worker.avatar).filter(Boolean)))) as string[],[rows,user.role]);
  useEffect(()=>{
  ...
  const renderWorkerAvatars=(item:any,compact=false)=>{
    ...
    return <div className={`sv2-worker-avatars ${compact?'compact':''}`} aria-label="Profilbilder der zugewiesenen Mitarbeiter">
      {assigned.slice(0,limit).map((worker:any)=>{
        const content=<><span>{workerInitials(worker)}</span>{user.role!=='client'&&worker.avatar&&<img src={worker.avatar} alt="" loading="eager" decoding="async" onError={e=>{e.currentTarget.style.display='none';}}/>}</>;
  ...
  return <div className="sv2">
    ...
    {view==='day'&&<div className="sv2-day-wrap" data-testid="schedule-day-view"><div className="sv2-single-day"><header><div><small>{keyLabel(anchor,{weekday:'long'})}</small><h2>{keyLabel(anchor,{day:'2-digit',month:'long',year:'numeric'})}</h2></div><span>{(rowsByDay[anchor]||[]).length} Einsätze</span></header><div className="sv2-single-day-events">{renderCustomerSeparated(rowsByDay[anchor]||[],item=>renderMini(item))}{!(rowsByDay[anchor]||[]).length&&<div className="sv2-no-events">Keine Einsätze an diesem Tag.</div>}</div></div></div>}

    {view==='week'&&<div className="sv2-week-wrap" data-testid="schedule-week-view"><div className="sv2-week-grid">{weekDays.map(key=><section className={`sv2-week-day ${key===berlinDate()?'is-today':''}`} key={key}><header><b>{keyLabel(key,{weekday:'short'})}</b><span>{keyLabel(key,{day:'2-digit',month:'2-digit'})}</span></header><div className="sv2-day-events">{renderCustomerSeparated(rowsByDay[key]||[],item=>renderMini(item))}{!(rowsByDay[key]||[]).length&&<small className="sv2-no-events">Keine Einsätze</small>}</div></section>)}</div></div>}
    ...
    <IonModal isOpen={modal} onDidDismiss={()=>setModal(false)}><div className="sv2-modal"><div className="sv2-modal-head"><h2>{editing?'Personalbedarf bearbeiten':'Personalbedarf anlegen'}</h2><IonButton fill="clear" onClick={()=>setModal(false)}>Schließen</IonButton></div><div className="sv2-form">
      ...
      <IonSelect className="full" multiple interface="alert" fill="outline" label={editing?'Mitarbeiter auswählen / ändern':'Mitarbeiter direkt zuweisen (optional)'} labelPlacement="floating" value={form.workers||[]} onIonChange={e=>{const selected=Array.isArray(val(e))?val(e):[];const limit=Math.max(1,Number(form.required_count||1));if(selected.length>limit)setToast(`Maximal ${limit} Mitarbeiter auswählbar.`);setForm({...form,workers:selected.slice(0,limit)});}}>{workers.map(worker=><IonSelectOption key={worker.id} value={worker.id}>{workerLabel(worker)} · {worker.employee_number}</IonSelectOption>)}</IonSelect>
      ...
      <IonSelect className="full" multiple interface="alert" fill="outline" label="Zeitplan · Sichtbare Mitarbeitergruppen" labelPlacement="floating" value={form.schedule_groups||[]} onIonChange={e=>{const raw=Array.isArray(val(e))?val(e):[];const groups=raw.includes('service')?['service']:raw;const currentPosition=positions.find(x=>x.id===form.position);setForm({...form,schedule_groups:groups,position:groups.length===1&&groups[0]==='service'&&isHotelPositionName(currentPosition?.name)?undefined:form.position});}}>{SCHEDULE_GROUPS.map(([key,label])=><IonSelectOption key={key} value={key}>{label}</IonSelectOption>)}</IonSelect>
