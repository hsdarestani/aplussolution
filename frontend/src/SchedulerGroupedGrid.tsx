...
function dateKey(value:Date|string,zone?:string){const x=typeof value==='string'?new Date(value):value;if(!zone)return `${x.getFullYear()}-${String(x.getMonth()+1).padStart(2,'0')}-${String(x.getDate()).padStart(2,'0')}`;try{const parts=new Intl.DateTimeFormat('en-CA',{year:'numeric',month:'2-digit',day:'2-digit',timeZone:zone}).formatToParts(x);const get=(type:string)=>parts.find(part=>part.type===type)?.value||'';return `${get('year')}-${get('month')}-${get('day')}`}catch{return `${x.getFullYear()}-${String(x.getMonth()+1).padStart(2,'0')}-${String(x.getDate()).padStart(2,'0')}`}}
...
function time(v:string,zone?:string){try{return new Date(v).toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit',timeZone:zone})}catch{return new Date(v).toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'})}}
...
export default function SchedulerGroupedGrid({rows,mode,anchor,groupBy,onMove,onInspect,selected,onToggleSelect}:{rows:any[];mode:CalendarMode;anchor:Date;groupBy:'users'|'positions';onMove:(shift:any,targetDay:Date)=>Promise<void>|void;onInspect:(shift:any)=>void;selected:Set<string>;onToggleSelect:(id:string)=>void}){
  ...
  return <div className="scheduler-matrix-wrap"><div className="scheduler-matrix" style={{gridTemplateColumns:`190px repeat(${days.length}, minmax(145px,1fr))`}}>
    ...
    {groups.map(group=><React.Fragment key={group.id}><div className={`scheduler-matrix-group ${group.id==='__open__'?'open':''}`}><b>{group.name}</b><small>{group.rows.length} Schicht(en)</small></div>{days.map(day=>{const dayRows=group.rows.filter(x=>dateKey(x.starts_at,x.display_time_zone)===dateKey(day));return <div className="scheduler-matrix-cell" key={`${group.id}-${dateKey(day)}`} onDragOver={e=>e.preventDefault()} onDrop={e=>drop(e,day)}>{dayRows.map(shift=>{const pending=(shift.assignments||[]).some((item:any)=>item.confirmation_status==='pending');return <article draggable key={`${group.id}-${shift.id}`} className={`scheduler-mini-shift ${shift.status} ${selected.has(shift.id)?'selected':''} ${pending?'unconfirmed':''}`} style={{borderLeftColor:shift.display_color||undefined,borderLeftWidth:'4px'}} onDragStart={e=>{e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/shift-id',shift.id)}}><button aria-label="Auswählen" onClick={e=>{e.stopPropagation();onToggleSelect(shift.id)}}>{selected.has(shift.id)?'✓':'○'}</button><div onClick={()=>onInspect(shift)}><span>{time(shift.starts_at,shift.display_time_zone)}–{time(shift.ends_at,shift.display_time_zone)}</span><b>{groupBy==='users'?shift.position_name:shift.client_name}</b><small>{shift.location_name}</small></div>{Number(shift.open_count||0)>0&&<IonBadge color="warning">{shift.open_count} offen</IonBadge>}{pending&&<IonBadge color="warning">Unbestätigt</IonBadge>}</article>})}{!dayRows.length&&<span className="scheduler-matrix-empty">·</span>}</div>})}</React.Fragment>)}
