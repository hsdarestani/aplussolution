...
export default function SchedulerCompletionPanel({ open, onClose, user, rows, locations, positions, workers, display, onDisplayChange, onChanged }: Props) {
  ...
  async function saveColor() {
    if (!colorForm.target_id) { setError('Bitte Schicht oder Einsatzort auswählen.'); return; }
    setBusy(true); setError('');
    try {
  ...
  const activeTargetOptions = useMemo(() => colorForm.target_type === 'shift'
    ? rows.map((row) => ({ id: row.id, name: `${new Date(row.starts_at).toLocaleDateString('de-DE')} · ${row.position_name} · ${row.location_name}` }))
    : locations.map((row) => ({ id: row.id, name: row.name })), [colorForm.target_type, rows, locations]);
