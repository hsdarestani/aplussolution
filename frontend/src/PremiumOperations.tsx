...
export default function PremiumOperations({ user }: { user: User }) {
  ...
  return <section className="operations-panel premium-ops" data-testid="premium-operations-panel">
    ...
    {admin&&<details className="premium-box"><summary>API, Webhooks & Unternehmens-SSO</summary><div className="premium-enterprise"><div><b>Öffentliche API</b><span>{apiKeys.length} aktive/gespeicherte Schlüssel</span><IonButton size="small" fill="outline" onClick={issueApiKey}>Neuen API-Schlüssel</IonButton></div><div><b>SAML 2.0 / SSO</b><span>{saml.enabled?'Identitätsanbieter ist konfiguriert':'Code aktiv, Zugangsdaten des Identitätsanbieters fehlen'}</span><IonBadge color={saml.enabled?'success':'medium'}>{saml.enabled?'Aktiv':'Konfigurieren'}</IonBadge></div></div>{issuedKey&&<div className="premium-key"><b>Nur jetzt sichtbar:</b><code>{issuedKey}</code></div>}<div className="premium-fields"><IonInput className="premium-wide" fill="outline" type="url" label="Webhook-HTTPS-URL" labelPlacement="floating" value={webhookUrl} onIonInput={event=>setWebhookUrl(String(event.detail.value||''))}/></div><div className="premium-actions"><IonButton fill="outline" onClick={createWebhook}>Webhook anlegen</IonButton></div><div className="premium-result">{webhooks.length} Webhook-Abos · signierte HMAC-Auslieferung mit Wiederholungsversuch</div></details>}
  </section>;
}
