...
function operationsOverview() {
  return {
    ...
    notifications: [{
      id: 'n1',
      title: 'Berlin-Zeitprüfung',
      body: 'Zeitstempel muss in Deutschland angezeigt werden.',
    ...
    readiness: {
      ...
      apple_login: true,
      email_delivery: true,
      company_legal_data: true,
      aueg_data: true,
      final_contract_set_complete: true,
      android_signing_configured: true,
      ios_signing_configured: true,
      store_api_credentials_configured: true,
      contract_templates: {},
    },
    conflicts: [],
    ...
    overtime_risks: [],
    pending_swaps: 2,
    unapproved_time_entries: 3,
    contracts_due_30: 4,
    estimated_monthly_labor_cost: '1234.50',
  };
}
...
function catalog() {
  return {
    count: 8,
    complete: true,
    recovery: { complete: true, expected: 8, installed: 8, recovered: 0, missing: [], ambiguous: [], invalid: [] },
...
async function fulfillCommon(route: any, degradedWorkingTime = false) {
  ...
  if (url.pathname.endsWith('/working-time/settings/')) {
    if (degradedWorkingTime) return route.fulfill({ status: 503, json: { detail: 'temporary outage' } });
...
test('KPI cards stay visible and German timestamps use Europe/Berlin', async ({ page }) => {
  ...
  await openSteuerzentrale(page);

  const cards = page.locator('.operations-stats ion-card');
  await expect(cards).toHaveCount(5);
  await expect(cards.nth(0)).toContainText('Planungsrisiken');
  await expect(cards.nth(0)).toContainText('0');
  await expect(cards.nth(1)).toContainText('Offene Tauschanfragen');
  await expect(cards.nth(1)).toContainText('2');
  await expect(cards.nth(2)).toContainText('Ungeprüfte Zeiten');
  await expect(cards.nth(2)).toContainText('3');
  await expect(cards.nth(3)).toContainText('Verträge ≤ 30 Tage');
  await expect(cards.nth(3)).toContainText('4');
  await expect(cards.nth(4)).toContainText('Geplante Lohnkosten');
  await expect(cards.nth(4)).toContainText(/1[.\s]234,50\s*€/);

  for (let index = 0; index < 5; index += 1) {
    const content = cards.nth(index).locator('ion-card-content');
...
test('one auxiliary 503 no longer breaks the whole Steuerzentrale', async ({ page }) => {
  await installAdmin(page);
  await page.route(apiBase, (route) => fulfillCommon(route, true));
  await openSteuerzentrale(page);

  await expect(page.getByTestId('working-time-panel')).toBeVisible();
  await expect(page.getByText('Arbeitszeitkonto', { exact: true })).toBeVisible();
